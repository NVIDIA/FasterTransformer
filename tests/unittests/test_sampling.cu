...
void printTensors(std::unordered_map<std::string, Tensor>* map, size_t limit = 8) {
    FT_LOG_INFO("Tensors:");
    for (auto& kv : *map) {
        Tensor t = kv.second;
...
private:
    ...
    std::unordered_map<std::string, Tensor>* createInputTensors(int* topk,
                                                                size_t topk_size,
                                                                float* topp,
                                                                size_t topp_size,
                                                                float* temperature,
                                                                float* repetition_penalty)
    {
        ...
        std::unordered_map<std::string, Tensor>* input_tensors = new std::unordered_map<std::string, Tensor>();
        if (topk != nullptr) {
    ...
    std::unordered_map<std::string, Tensor>* createOutputTensors() {
        // construct common output tensors
        std::unordered_map<std::string, Tensor>* output_tensors = new std::unordered_map<std::string, Tensor>();
    ...
    bool checkResult(std::string name, int* d_output_ids, std::vector<std::set<int>>& expected_ids) {
        ...
        FT_LOG_DEBUG("check...%6s : %s (failures: %d / %d)",
                     failures == 0 ? "....OK" : "FAILED", name.c_str(),
                     failures, max_seq_len * batchxbeam);
        return failures == 0;
    }
                       ...
                       float* repetition_penalty)
    {
        ...
        std::string tag = fmtstr(
            "Test %s T=%s", name.c_str(), std::is_same<T, float>::value ? "fp32" : "fp16");
        bool passed = true;
        for (unsigned long long seed = 0; seed < max_seed; ++seed) {
            this->setup(seed);
            size_t step = max_input_len;
            uint ite = 0;
            std::unordered_map<std::string, Tensor>* input_tensors = createInputTensors(
            ...
            std::unordered_map<std::string, Tensor>* output_tensors = createOutputTensors();
...
public:
    ...
    void testTopK() {
        ...
        bool passed = this->testSampling(
            "TopK", expected_output_ids, &top_k, 1, nullptr, 0, nullptr, nullptr);
        EXPECT_TRUE(true);
    }
    ...
    void testBatchTopK() {
        ...
        bool passed = this->testSampling(
            "BatchTopK", expected_output_ids, top_ks, batch_size, nullptr, 0, nullptr, nullptr);
        delete[] top_ks;
        EXPECT_TRUE(passed);
    }
    ...
    void testTopP() {
        ...
        bool passed = this->testSampling(
            "TopP", expected_output_ids, nullptr, 0, &top_p, 1, nullptr, nullptr);
        EXPECT_TRUE(true);
    }
    ...
    void testBatchTopP() {
        ...
        bool passed = this->testSampling(
            "BatchTopP", expected_output_ids, nullptr, 0, top_ps, batch_size, nullptr, nullptr);
        delete[] top_ps;
        EXPECT_TRUE(passed);
    }
    ...
    void testTopKTopP() {
        ...
        bool passed = this->testSampling(
            "TopP", expected_output_ids, &top_k, 1, &top_p, 1, nullptr, nullptr);
        EXPECT_TRUE(true);
    }
    ...
    void testBatchTopKTopP() {
        ...
        bool passed = this->testSampling(
            name, expected_output_ids, top_ks, batch_size, &top_p, 1, nullptr, nullptr);
        delete[] top_ks;
        EXPECT_TRUE(passed);
    }
    ...
    void testTopKBatchTopP() {
        ...
        bool passed = this->testSampling(
            name, expected_output_ids, &top_k, 1, top_ps, batch_size, nullptr, nullptr);
        delete[] top_ps;
        EXPECT_TRUE(passed);
    }
    ...
    void testInvalidArgsZeroTopK() {
        ...
        bool passed = this->testSampling(
            name, expected_output_ids, &top_k, 1, nullptr, 0, nullptr, nullptr);
        EXPECT_TRUE(passed);
    }
    ...
    void testInvalidArgsZeroTopP() {
        ...
        bool passed = this->testSampling(
            name, expected_output_ids, nullptr, 0, &top_p, 1, nullptr, nullptr);
        EXPECT_TRUE(passed);
    }
    ...
    void testInvalidArgsZeroTopKTopP() {
        ...
        bool passed = this->testSampling(
            name, expected_output_ids, &top_k, 1, &top_p, 1, nullptr, nullptr);
        EXPECT_TRUE(passed);
    }
    ...
    void testInvalidArgsBatchTopKContainZero() {
        ...
        bool passed = this->testSampling(
            name, expected_output_ids, top_ks, batch_size, nullptr, 0, nullptr, nullptr);
        delete[] top_ks;
        EXPECT_TRUE(passed);
    }
