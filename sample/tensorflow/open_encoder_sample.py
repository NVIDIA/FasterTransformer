...
def encoder_sample(args_dict):
    ...
    with tf.variable_scope("transformer/encoder", reuse=tf.AUTO_REUSE):
        ...
        ft_encoder_result = ft_encoder_opennmt(inputs=from_tensor,
                                               encoder_args=encoder_args,
    ...
    ft_encoder_result = ft_encoder_result * tf.expand_dims(tf.sequence_mask(sequence_length, maxlen=max_seq_len, dtype=tf_datatype), axis=-1)
    ...
    with tf.Session(config=config) as sess:
        ...
        ft_encoder_result_val = sess.run(ft_encoder_result)

        cross_check("Encoder TF v.s. FT with tensor input", tf_encoder_result_val,
                    ft_encoder_result_val, atol_threshold)
        ...
        if args_dict['test_time'] == 1:
            ...
            op_time = time_test(sess, ft_encoder_result, ite)
        ...
        return (tf_encoder_result_val.reshape([-1]) - ft_encoder_result_val.reshape([-1])).max()
