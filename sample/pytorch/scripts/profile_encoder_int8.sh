...
echo -e "| Batch_size | Seq_len | TorchScript-FP16 <br/> Latency (ms) | FT-INT8-v${int8_mode} <br/> Latency (ms) | EFF-FT-INT8-v${int8_mode} <br/> Latency (ms) | FT-INT8-v${int8_mode} <br/> Speedup | EFF-FT-INT8-v${int8_mode} <br/> Speedup | " > $all_log
echo -e "|:----------:|:-------:|:-----------------------------------:|:----------------------------------------:|:--------------------------------------------:|:-----------------------------------:|:---------------------------------------:| " >> $all_log
...
do
    ...
    ./bin/encoder_gemm ${batch_size} ${seq_len} 12 64 1 ${int8_mode}
    
    tmp_log_ths=${logdir}/batchsize-${batch_size}-seq-${seq_len}-fp16-ths-log.log
    python pytorch/encoder_sample.py ${batch_size} 12 ${seq_len} 12 64 --fp16 --time --int8_mode ${int8_mode} 2>&1 | tee $tmp_log_ths
    ...
    echo "" | awk -v ths_time=$ths_time -v ft_time=$ft_time \
                        ...
                        ths_time, ft_time, eff_ft_time, ft_speedup, eff_ft_speedup }' >> $all_log
done
done
done # for int8_mode