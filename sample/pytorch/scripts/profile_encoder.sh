...
if [ -f "gemm_config.in" ] ; then
    ...
    if [ "$precision" = "fp16" ]; then
        python pytorch/encoder_sample.py ${batch_size} 12 ${seq_len} 12 64 --fp16 --time 2>&1 | tee $tmp_log_ths
    else
        python pytorch/encoder_sample.py ${batch_size} 12 ${seq_len} 12 64 --time 2>&1 | tee $tmp_log_ths
    fi
