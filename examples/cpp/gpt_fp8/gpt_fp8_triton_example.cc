                 ...
                 std::vector<void*>*          pointer_record)
{
    ...
    int* total_output_len_ptr = new int{max_input_len + request_output_len};
...
prepareRequest(std::string ini_name, const int node_id, const int gpu_count, std::vector<void*>* pointer_record)
{
    ...
    ft::read_start_ids(request_batch_size,
                       ...
                       end_id,
                       1,
                       "../examples/cpp/gpt_fp8/start_ids.csv");
                  ...
                  const int                                                         device_id)
{
    ...
    *output_tensors = (*model_instance)->forward(request);
    // fastertransformer::print_to_screen((int*)output_tensors[0].get()->at("output_ids").data, 10);
    return 0;
}
...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    std::string ini_name   = argc >= 2 ? std::string(argv[1]) : "../examples/cpp/gpt_fp8/gpt_config.ini";

    // step 1: Create model
    std::shared_ptr<AbstractTransformerModel> model            = AbstractTransformerModel::createGptFP8Model(ini_name);
