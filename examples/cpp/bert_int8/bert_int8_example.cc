...
int main(int argc, char **argv)
{
    ...
    if (atoi(argv[6]) == 0)
    {
        return bertINT8Example<float>(batch_size, num_layers, seq_len, head_num,
                                      size_per_head, int8_mode, is_remove_padding, allow_gemm_test);
    }
    else if (atoi(argv[6]) == 1)
    {
        return bertINT8Example<half>(batch_size, num_layers, seq_len, head_num,
                                     size_per_head, int8_mode, is_remove_padding, allow_gemm_test);
    }
    else
    {
        throw std::runtime_error(
            std::string("[FT][ERROR] is_fp16 should be 0 (use float)"
                        "or 1 (use half). \n "));
    }
}
                    ...
                    bool allow_gemm_test)
{
...
#ifdef SPARSITY_ENABLED
    cublasINT8MMWrapper cublas_wrapper = cublasINT8MMWrapper(cublaslt_handle, cusparselt_handle, stream, 
                                                             cublas_algo_map, cublas_wrapper_mutex, use_ORDER_COL32_2R_4R4);
#else
    cublasINT8MMWrapper cublas_wrapper = cublasINT8MMWrapper(cublaslt_handle, stream, 
                                                             cublas_algo_map, cublas_wrapper_mutex, use_ORDER_COL32_2R_4R4);
#endif

    std::vector<BertLayerINT8Weight<T>> bert_layer_weights(num_layers, BertLayerINT8Weight<T>(hidden_units, inter_size));

    AttentionType attention_type =
        getAttentionTypeINT8<T>(size_per_head, sm,
                                is_remove_padding, seq_len, int8_mode);

    BertINT8<T> bert_int8 = BertINT8<T>(
    ...
    cudaMemcpy(d_sequence_lengths, h_sequence_lengths,
    ...
    std::vector<Tensor> input_tensors = std::vector<Tensor>{
        Tensor{MEMORY_GPU, getTensorType<T>(),
               std::vector<size_t>{batch_size, seq_len,
                                   (size_t)(head_num * size_per_head)},
               from_tensor},
    ...
    printf("[INFO] batch_size %ld seq_len %ld layer %ld "
           ...
           batch_size, seq_len, num_layers, total_time / ite, ite);
