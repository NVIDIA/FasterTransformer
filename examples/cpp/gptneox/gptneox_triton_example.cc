...
prepareRequest(std::string ini_name, const int node_id, const int gpu_count, std::vector<void*>* pointer_record)
{
    ...
    const int start_id = reader.GetInteger("gptneox_20B", "start_id");
    const int end_id   = reader.GetInteger("gptneox_20B", "end_id");
    ...
    ft::read_word_list("../examples/cpp/gptneox/bad_words.csv", v_bad_words);
...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    std::string ini_name   = argc >= 2 ? std::string(argv[1]) : "../examples/cpp/gptneox/gptneox_config.ini";

    // step 1: Create model
    std::shared_ptr<AbstractTransformerModel> model            = AbstractTransformerModel::createGptNeoXModel(ini_name);
