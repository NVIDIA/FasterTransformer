...
class LinearActivation(nn.Module):
    ...
    def forward(self, input):
        ...
        if not self.bias is None:
            ...
            if QUANT and self.do_quant:
                return self.act_fn(self._aftergemm_quantizer(F.linear(input, weight, self.bias)))
            else:
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.qkv = LinearActivation(dim, dim * 3, act='noact', do_quant=True)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = LinearActivation(dim, dim, act='noact', do_quant=True)
    ...
    def forward(self, x, mask=None):
        ...
        # x_int8 = self.proj._input_quantizer(x) * 127.0 / self.proj._input_quantizer._amax
        # print(x_int8.round()[0, 0, 0:32])
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
