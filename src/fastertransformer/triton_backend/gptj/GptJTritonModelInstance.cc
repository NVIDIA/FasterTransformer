...
{
    ft::FT_CHECK(beam_width_ == 1);  // TODO(bhsueh) disable the beam search
}

template<typename T>
std::vector<ft::Tensor>
GptJTritonModelInstance<T>::convert_inputs(std::shared_ptr<std::vector<triton::Tensor>> input_tensors)
{
...
GptJTritonModelInstance<T>::convert_outputs(const std::vector<ft::Tensor>& output_tensors)
{
...
template<typename T>
void GptJTritonModelInstance<T>::allocateBuffer(const size_t request_batch_size, const size_t total_output_len)
{
