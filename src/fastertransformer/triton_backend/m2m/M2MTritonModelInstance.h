...
template<typename T>
struct M2MTritonModelInstance: AbstractTransformerModelInstance {

    M2MTritonModelInstance(std::unique_ptr<ft::M2MEncoder<T>>                       m2m_encoder,
...
private:
    const std::unique_ptr<ft::M2MEncoder<T>>                       m2m_encoder_;
    const std::shared_ptr<ft::M2MEncoderWeight<T>>                 m2m_encoder_weight_;
    const std::unique_ptr<ft::M2MDecoding<T>>                      m2m_decoding_;
    const std::shared_ptr<ft::M2MDecodingWeight<T>>                m2m_decoding_weight_;
