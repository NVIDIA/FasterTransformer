...
void compressInt8Matrix(void* output, const void* input, const int m, const int k, cudaStream_t stream)
{
  ...
  CHECK_CUSPARSE( cusparseLtStructuredDescriptorInit(
                                          &_cusparseLtHandle, &matA, m,
                                          k, k, alignment,
                                          CUDA_R_8I, CUSPARSE_ORDER_ROW,
                                          CUSPARSELT_SPARSITY_50_PERCENT) )
  CHECK_CUSPARSE( cusparseLtSpMMACompress2(&_cusparseLtHandle, &matA, true, CUSPARSE_OPERATION_NON_TRANSPOSE, input, output, stream) )
...
Tensor weight_quantize(Tensor weight, Tensor quant_max, bool sparse) {
  ...
  CHECK_TH_CUDA(weight); CHECK_TH_CUDA(weight);
  ...
  CHECK_TH_CUDA(quant_max); CHECK_TH_CUDA(quant_max);
  ...
  if (sparse) {
    ...
    invokeQuantizeWeight(tmp_out, weight_, quant_max_, n, k, format, stream);
