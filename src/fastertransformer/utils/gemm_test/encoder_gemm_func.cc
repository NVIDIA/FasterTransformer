...
template <typename T>
void generate_encoder_gemm_config(int batch_size, int seq_len, int head_num,
                                  int size_per_head, void *buffer_in,
                                  bool isAppend) {
    ...
    if (!isAppend) {
        ...
        if (config.size()
            >= (MAX_CONFIG_NUM * GEMM_NUM
                + 1))  // 6 cublas/cublasLt, first row is not included
        {
    ...
    for (int i = 0; i < gemm_num; ++i) {
            ...
            for (int ite = 0; ite < ites; ++ite) {
                if (i < 3) {
                    status = cublasGemmEx(
                        ...
                        &alpha, d_B, BType, n, d_A, AType, k, &beta, d_C, CType,
                        n, computeType, static_cast<cublasGemmAlgo_t>(algo));
                } else if (i == 3) {
                    status = cublasGemmStridedBatchedEx(
                        cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, seq_len,
                        seq_len, size_per_head, &alpha, d_B, BType,
                        size_per_head, seq_len * size_per_head, d_A, AType,
                        size_per_head, seq_len * size_per_head, &beta, d_C,
                        CType, seq_len, seq_len * seq_len,
                        batch_size * head_num, computeType,
                        static_cast<cublasGemmAlgo_t>(algo));
                } else if (i == 4) {
                    status = cublasGemmStridedBatchedEx(
                        cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, size_per_head,
                        seq_len, seq_len, &alpha, d_B, BType, size_per_head,
                        seq_len * size_per_head, d_A, AType, seq_len,
                        seq_len * seq_len, &beta, d_C, CType, size_per_head,
                        seq_len * size_per_head, batch_size * head_num,
                        computeType, static_cast<cublasGemmAlgo_t>(algo));
                } else if (i == 5) {
                    status = cublasGemmBatchedEx(
                        cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, n, m, k,
                        &alpha, (const void *const *)dBarray, BType, n,
                        (const void *const *)dAarray, AType, k, &beta,
                        (void *const *)dCarray, CType, n, 3, computeType,
                        static_cast<cublasGemmAlgo_t>(algo));
                }
        ...
        int is_fp16 = 0;
        if (sizeof(T) == sizeof(half)) is_fp16 = 1;
        ...
        if (i < 3 && is_fp16 == 1) {
            ...
            LtHgemmCustomFind<T, T>(ltHandle, batch_size, seq_len, head_num,
                                    size_per_head, n, m, k, &alpha, d_B, d_A,
                                    &beta, d_C, cublas_workspace, workSpaceSize,
                                    fd, perfResults, ALGO_COMBINATIONS);
            if (perfResults[0].time < exec_time) {
                printPerfStructure(batch_size, seq_len, head_num, size_per_head,
                                   n, m, k, perfResults[0], fd, is_fp16, 0);
                ...
                fprintf(
                    ...
                    is_fp16 ? HALF_DATATYPE : FLOAT_DATATYPE, batchCount[i], n,
                    m, k, fast_algo, exec_time);
            }
            ...
            fprintf(
                ...
                "%d %d %d %d %d ### %d %d %d %d %d -1 -1 -1 -1 -1 -1 -1 %f\n",
                batch_size, seq_len, head_num, size_per_head,
                is_fp16 ? HALF_DATATYPE : FLOAT_DATATYPE, batchCount[i], n, m,
                k, fast_algo, exec_time);
        }
    ...
    if (do_sparse_test && sizeof(T) == sizeof(half)) {
        ...
        for(int i = 0; i < spgemm_num; ++i) {
            ...
            T* dA_compressed;
            {
                ...
                CHECK_CUSPARSE( cusparseLtSpMMAPrune2(&handle, &matA, true, opA, d_A, d_A, CUSPARSELT_PRUNE_SPMMA_STRIP, stream) )
            ...
            for (int alg = 0; alg < 4; ++alg) {
                ...
                for (int ite = 0; ite < ites; ++ite) {
                    ...
                    CHECK_CUSPARSE( cusparseLtMatmul(&handle, &plan, &alpha2, dA_compressed, d_B,
                                                        &beta2, d_C, d_C, d_workspace, streams, num_streams) )
            ...
            fprintf(fd, "%d %d %d %d %d ### %d %d %d %d %d %f\n",
                    batch_size, seq_len, head_num, size_per_head, HALF_DATATYPE, batchCount[i], m, n, k, fast_algo, exec_time);
...
template void generate_encoder_gemm_config<float>(int batch_size, int seq_len,
                                                  int head_num,
                                                  int size_per_head,
                                                  void *buffer, bool isAppend);
template void generate_encoder_gemm_config<half>(int batch_size, int seq_len,
                                                 int head_num,
                                                 int size_per_head,
                                                 void *buffer, bool isAppend);
