...
public:
    ...
    void Compute(tf::OpKernelContext* context) override
    {
        ...
        ft::cublasINT8MMWrapper cublas_wrapper =
            ft::cublasINT8MMWrapper(cublaslt_handle, stream, cublas_algo_map_, this->get_cublas_wrapper_mutex(), use_ORDER_COL32_2R_4R4_);
            
        ft::Allocator<ft::AllocatorType::TF> allocator(context, stream);

        if (!set_weight_)
        {
            const int scale_list_size = ACTIVATION_AMAX_NUM + 9*head_num_*size_per_head_ + INT8O_GEMM_NUM + TRT_AMAX_NUM + SCALE_RESERVE_NUM;
	    h_scale_list_ = (float*)malloc(num_layer_*scale_list_size*sizeof(float));
            ...
            for (int i = 0; i < num_layer_; i++) {
                ...
                this->get_tensor(context, 3 + num_layer_ + i, &bert_layer_weights_[i].attention_weights.query_weight.bias);
                this->get_tensor(context, 3 + num_layer_ * 2 + i, &bert_layer_weights_[i].attention_weights.key_weight.kernel);
                this->get_tensor(context, 3 + num_layer_ * 3 + i, &bert_layer_weights_[i].attention_weights.key_weight.bias);
                this->get_tensor(context, 3 + num_layer_ * 4 + i, &bert_layer_weights_[i].attention_weights.value_weight.kernel);
                this->get_tensor(context, 3 + num_layer_ * 5 + i, &bert_layer_weights_[i].attention_weights.value_weight.bias);
                this->get_tensor(context,3 + num_layer_ * 6 + i, &bert_layer_weights_[i].attention_weights.attention_output_weight.kernel);
                this->get_tensor(context, 3 + num_layer_ * 7 + i, &bert_layer_weights_[i].attention_weights.attention_output_weight.bias);
                ...
                this->get_tensor(context, 3 + num_layer_ * 10 + i, &bert_layer_weights_[i].ffn_weights.intermediate_weight.kernel);
                this->get_tensor(context, 3 + num_layer_ * 11 + i, &bert_layer_weights_[i].ffn_weights.intermediate_weight.bias);
                this->get_tensor(context, 3 + num_layer_ * 12 + i, &bert_layer_weights_[i].ffn_weights.output_weight.kernel);
                this->get_tensor(context, 3 + num_layer_ * 13 + i, &bert_layer_weights_[i].ffn_weights.output_weight.bias);
		...
		bert_layer_weights_[i].scale_list_.d_scale_list_ = reinterpret_cast<const float *>(context->input(3 + num_layer_ * 16 + i).flat<float>().data());
                ...
                bert_layer_weights_[i].scale_list_.p4_offset_ = ACTIVATION_AMAX_NUM + 9*head_num_*size_per_head_ + INT8O_GEMM_NUM;
                ...
                cudaMemcpy(h_scale_list_ + i*scale_list_size,
                    ...
                    sizeof(float)*scale_list_size, cudaMemcpyDeviceToHost);
