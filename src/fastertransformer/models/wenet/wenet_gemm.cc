...
{
    if (argc != 6) {
        printf("[ERROR] wenet_gemm batch_size seq_len head_number size_per_head data_type \n");
        printf("e.g. ./bin/wenet_gemm 1 63 4 64 1\n");
        return 0;
    }
    ...
    const int                int8_mode     = 0;                                               // not support int8 yet
    ...
    // TODO add gemm test for Wenet decoder
