...
{
    gpt_context_decoder_ = new GptJContextDecoder<T>(max_batch_size_ * beam_width_,
                                                     max_input_len_,
                                                     head_num_,
                                                     size_per_head_,
                                                     inter_size_,
    ...
    gpt_decoder_ = new GptJDecoder<T>(max_batch_size_ * beam_width_,
                                      head_num_,
                                      size_per_head_,
                                      inter_size_,
                                      num_layer_,
...
template<typename T>
bool GptJ<T>::isValidBatchSize(size_t batch_size)
{
...
template<typename T>
bool GptJ<T>::isValidSeqLen(size_t seq_len)
{
...
template<typename T>
bool GptJ<T>::isValidInputSeqLen(size_t seq_len)
{
...
GptJ<T>::GptJ(size_t max_batch_size,
    ...
    tensor_para_size_(1),
    tensor_para_rank_(0),
    tensor_para_comm_(nullptr),
    local_head_num_(head_num_ / tensor_para_size_),
    pipeline_para_size_(1),
    pipeline_para_rank_(0),
    pipeline_para_comm_(nullptr)
{
