                                   ...
                                   const std::vector<GptJDecoderLayerWeight<T>>* gpt_decoder_layer_weight)
{
    ...
    const int local_batch_size = getLocalBatchSize(batch_size, seq_len, layer_para_size_);
    ...
    for (int ite = 0; ite < iteration_num; ite++) {
        for (int l = 0; l < num_layer_; l++) {
            ...
            T* layer_input = (
                (l == 0) ? decoder_input : decoder_layer_output_
            ) + ite * local_batch_size * seq_len * hidden_units_;
            T* layer_output = (
                (l == num_layer_ - 1) ? decoder_output : decoder_layer_output_
            ) + ite * local_batch_size * seq_len * hidden_units_;

            if (isFirstLayerParallelId(l) && layer_para_rank_ != 0 && layer_para_size_ > 1) {
                int data_size = local_batch_size * seq_len * hidden_units_ / tensor_para_size_;
            ...
            if (is_final == false) {
                ...
                if (isLastLayerParallelId(l) && layer_para_rank_ != layer_para_size_ - 1 && layer_para_size_ > 1) {
                    int data_size = local_batch_size * seq_len * hidden_units_ / tensor_para_size_;
