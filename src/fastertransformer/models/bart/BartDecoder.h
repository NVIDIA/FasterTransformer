...
template<typename T>
class BartDecoder: public BaseLayer {
private:
    // buffer handling
...
public:
    BartDecoder(size_t                              max_batch_size,
                size_t                              head_num,
    ...
    BartDecoder(BartDecoder<T> const& decoder);

    ~BartDecoder();

    void forward(std::vector<Tensor>*                           output_tensors,
                 const std::vector<Tensor>*                     input_tensors,
                 const std::vector<BartDecoderLayerWeight<T>*>* decoder_layer_weights);
