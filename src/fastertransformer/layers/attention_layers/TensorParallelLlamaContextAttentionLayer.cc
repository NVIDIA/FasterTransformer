...
template<typename T>
void TensorParallelLlamaContextAttentionLayer<T>::forward(TensorMap*                output_tensors,
                                                        ...
                                                        const AttentionWeight<T>* attention_weights)
{
    ...
    LlamaContextAttentionLayer<T>::forward(output_tensors, input_tensors, attention_weights);
    ...
    if (do_all_reduce_ && tensor_para_.world_size_ > 1) {
        if (!use_custom_all_reduce_kernel) {
            ftNcclAllReduceSum(attention_out, attention_out, size, tensor_para_, LlamaContextAttentionLayer<T>::stream_);
        }
        else {
            custom_all_reduce_comm_->customAllReduce(size, LlamaContextAttentionLayer<T>::stream_);
        }
...
template<typename T>
TensorParallelLlamaContextAttentionLayer<T>::TensorParallelLlamaContextAttentionLayer(
    ...
    LlamaContextAttentionLayer<T>(max_batch_size,
                                max_seq_len,
                                head_num,
                                kv_head_num,
                                size_per_head,
...
template<typename T>
TensorParallelLlamaContextAttentionLayer<T>::TensorParallelLlamaContextAttentionLayer(
    TensorParallelLlamaContextAttentionLayer<T> const& attention_layer):
    LlamaContextAttentionLayer<T>(attention_layer),
...
template class TensorParallelLlamaContextAttentionLayer<float>;
template class TensorParallelLlamaContextAttentionLayer<half>;
#ifdef ENABLE_BF16
template class TensorParallelLlamaContextAttentionLayer<__nv_bfloat16>;
#endif
