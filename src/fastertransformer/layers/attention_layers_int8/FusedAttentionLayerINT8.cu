...
template <typename T>
void invokeTrtAddQkvBiasInt32Iint8O(
    int8_t* output,
    const int32_t* Q,
    const T* bias_Q,
    const T* bias_K,
    ...
    trt_add_QKV_bias_COL32_int32IInt8O<<<grid, block, 0, stream>>>((char4*)output, (const int4*)Q,
                                                                   bias_Q, bias_K, bias_V,
                                                                   input_deQFactor_div127_ptr,
                                                                   q_weight_amax, 
                                                                   k_weight_amax,
                                                                   v_weight_amax,
                                                                   1.f/mScaleQkv, token_num, 
                                                                   head_num, size_per_head, head_num_x_size_per_head);
}
...
template<typename T>
void invokeTrtAddQkvBiasInt8IO(
    int8_t* output,
    const int8_t* Q,
    const T* bias_Q,
    const T* bias_K,
    ...
    trt_add_QKV_bias_COL32_int8IO<<<grid, block, 0, stream>>>((char4*)output, (const char4*)Q,
                                                              bias_Q, bias_K, bias_V,
                                                              q_input_deQFactor_ptr, k_input_deQFactor_ptr, v_input_deQFactor_ptr,
                                                              1.0f/mScaleQkv, token_num, 
                                                              head_num, size_per_head, head_num_x_size_per_head);
}
...
template <typename T>
__global__
void trt_add_QKV_bias_ROW_int8IO(char4* output, const char4* QKV,  
                                       const T* bias_Q, const T* bias_K, const T* bias_V, 
                                       const float *q_input_deQFactor_ptr, const float *k_input_deQFactor_ptr, 
                                       const float *v_input_deQFactor_ptr, const float qkv_output_scale,
                                       const int valid_word_num, const int head_num, const int size_per_head,
    ...
    if (qual) {
        ...
        const int input_id = (qkv_id * valid_word_num * head_num_x_size_per_head + seq_id * head_num_x_size_per_head + hidden_id) >> 2;
...
template<typename T>
void invokeTrtAddQkvBiasInt8IORow(
    int8_t* output,
    const int8_t* Q,
    const T* bias_Q,
    const T* bias_K,
    ...
    trt_add_QKV_bias_ROW_int8IO<<<grid, block, 0, stream>>>((char4*)output, (const char4*)Q,
                                                                  bias_Q, bias_K, bias_V,
                                                                  q_input_deQFactor_ptr, k_input_deQFactor_ptr, v_input_deQFactor_ptr,
                                                                  1.0f/mScaleQkv, token_num, 
                                                                  head_num, size_per_head, head_num_x_size_per_head);
                                         ...
                                         const AttentionWeight<T> *attention_weights)
{
    ...
    if (int8_mode_ == 1) {
        ...
        if (fusedINT8QKV_type == 0) {
            cublas_wrapper->Gemm(Q_int_buf_, 1, m, n, k, 0, 0, 0,
                                 from_tensor, (int8_t*)(attention_weights->query_weight.kernel));
            cublas_wrapper->Gemm(K_int_buf_, 1, m, n, k, 0, 0, 0,
                                 from_tensor, (int8_t*)(attention_weights->key_weight.kernel));
            cublas_wrapper->Gemm(V_int_buf_, 1, m, n, k, 0, 0, 0,
                                 from_tensor, (int8_t*)(attention_weights->value_weight.kernel));
        }
        else {
            int strideFactor = (fusedINT8QKV_type == 1) ? (sizeof(T)/sizeof(int8_t)) : 1; 
            cublas_wrapper->Gemm(Q_int_buf_, 3, m, n, k, 0, n*k*strideFactor, n*m,
                                 from_tensor, (int8_t*)(attention_weights->query_weight.kernel));
        }
        invokeTrtAddQkvBiasInt32Iint8O(qkv_buf_, Q_int_buf_, attention_weights->query_weight.bias,
                                       attention_weights->key_weight.bias, attention_weights->value_weight.bias,
                                       m, head_num_, size_per_head_,
...
#ifdef SPARSITY_ENABLED
        if (sparse_) {
            cublas_wrapper->SpGemm(n, m_padded, k,
...
#endif
        if (fusedINT8QKV_type == 0) {
            cublas_wrapper->Gemm((int8_t*)Q_int_buf_, 1, m, n, k, 0, 0, 0, 
                                 scale_list->h_scale_list_[scale_list->p3_offset_+0],
                                 from_tensor, (int8_t*)(attention_weights->query_weight.kernel));
            cublas_wrapper->Gemm((int8_t*)K_int_buf_, 1, m, n, k, 0, 0, 0,
                                 scale_list->h_scale_list_[scale_list->p3_offset_+1],
                                 from_tensor, (int8_t*)(attention_weights->key_weight.kernel));
            cublas_wrapper->Gemm((int8_t*)V_int_buf_, 1, m, n, k, 0, 0, 0,
                                 scale_list->h_scale_list_[scale_list->p3_offset_+2],
                                 from_tensor, (int8_t*)(attention_weights->value_weight.kernel));
        }
        else {
            int strideFactor = (fusedINT8QKV_type == 1) ? (sizeof(T)/sizeof(int8_t)) : 1; 
            cublas_wrapper->Gemm((int8_t*)Q_int_buf_, 3, m, n, k, 0, n*k*strideFactor, n*m, 
                                 scale_list->h_scale_list_[scale_list->p3_offset_+0],
                                 from_tensor, (int8_t*)(attention_weights->query_weight.kernel));
        }
#ifdef SPARSITY_ENABLED
        ...
        if (sparse_) {
            invokeTrtAddQkvBiasInt8IORow(qkv_buf_, (int8_t*)Q_int_buf_, attention_weights->query_weight.bias,
                                         attention_weights->key_weight.bias, attention_weights->value_weight.bias,
                                         m, head_num_, size_per_head_,
                                         &(scale_list->d_scale_list_[4+1]), &(scale_list->d_scale_list_[12+1]),
                                         &(scale_list->d_scale_list_[20+1]), scale_list->h_scale_list_[scale_list->p4_offset_]/127.0f, stream_);
        } else {
#endif
        invokeTrtAddQkvBiasInt8IO(qkv_buf_, (int8_t*)Q_int_buf_, attention_weights->query_weight.bias,
                                  attention_weights->key_weight.bias, attention_weights->value_weight.bias, m, head_num_, size_per_head_,
                                  &(scale_list->d_scale_list_[4+1]), &(scale_list->d_scale_list_[12+1]),
                                  &(scale_list->d_scale_list_[20+1]), scale_list->h_scale_list_[scale_list->p4_offset_]/127.0f, stream_);
#ifdef SPARSITY_ENABLED
        }
    ...
    if (int8_mode_ == 1) {
        cublas_wrapper->Gemm(attention_out, 1, m, n, k, 0, 0, 0,
                             qkv_buf_, (int8_t*)(attention_weights->attention_output_weight.kernel));
    }
    else if (int8_mode_ == 2 || int8_mode_ == 3) {
#ifdef SPARSITY_ENABLED
        if (sparse_) {
...
#endif
        cublas_wrapper->Gemm((int8_t*)attention_out, 1, m, n, k, 0, 0, 0, 
                             scale_list->h_scale_list_[scale_list->p3_offset_+5],
                             qkv_buf_, (int8_t*)(attention_weights->attention_output_weight.kernel));
#ifdef SPARSITY_ENABLED
        }
...
template <typename T>
FusedAttentionLayerINT8<T>::FusedAttentionLayerINT8(size_t max_batch_size, size_t max_seq_len,
    ...
    if ((sm_ == kSM_86 || sm_ == kSM_80 || sm_ == kSM_75 || sm_ == kSM_72) && size_per_head_ == 64)
        ...
        throw std::runtime_error(
            std::string("[FT][ERROR] FusedAttentionLayerINT8 not support \n"));
    }
...
void FusedAttentionLayerINT8<T>::allocateBuffer()
{
    if (is_allocate_buffer_ == false) {
        Q_int_buf_ = (int32_t*)allocator_->malloc(sizeof(int32_t) * max_batch_size_ * max_seq_len_ * hidden_units_ * 3, false);
        ...
        qkv_buf_ = (int8_t*)allocator_->malloc((sizeof(int8_t) * 3 * max_batch_size_ * max_seq_len_ * hidden_units_ + 3)/4*4, false);
        qkv_buf_2_ = (int8_t*)allocator_->malloc((sizeof(int8_t) * max_batch_size_ * max_seq_len_ * hidden_units_ + 3)/4*4, false);
