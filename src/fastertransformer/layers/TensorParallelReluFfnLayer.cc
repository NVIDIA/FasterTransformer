                                            ...
                                            const FfnWeight<T>* ffn_weights)
{
    ...
    if (do_all_reduce_ && tensor_para_.world_size_ > 1) {
        ...
        // cutlass)
        const bool cast_int_fp = ReluFfnLayer<T>::int8_mode_ == 2;
        if (cast_int_fp) {
            ffn_output_fp_ = reinterpret_cast<T*>(
                ReluFfnLayer<T>::allocator_->reMalloc(ffn_output_fp_, sizeof(T) * token_num * hidden_units, false));
...
template<typename T>
TensorParallelReluFfnLayer<T>::~TensorParallelReluFfnLayer()
{
    ReluFfnLayer<T>::allocator_->free((void**)&ffn_output_fp_);
}
