...
void invokeMappingRemovePaddingData(const int batch_size, const int seq_len, 
...
template <typename T>
void invokeAddQKBiasTransform(int8_t *q_buf, int8_t *k_buf, const int32_t* Q, const T* bias_Q, 
...
template
void invokeAddQKBiasTransform(int8_t *q_buf, int8_t *k_buf, const int32_t* Q, const float* bias_Q, 
...
template
void invokeAddQKBiasTransform(int8_t *q_buf, int8_t *k_buf, const int32_t* Q, const half* bias_Q, 
...
template <typename T>
void invokeAddQKBiasTransform(int8_t *q_buf, int8_t *k_buf, const int8_t* Q, const T* bias_Q, 
...
template
void invokeAddQKBiasTransform(int8_t *q_buf, int8_t *k_buf, const int8_t* Q, const float* bias_Q, 
...
template
void invokeAddQKBiasTransform(int8_t *q_buf, int8_t *k_buf, const int8_t* Q, const half* bias_Q, 
...
//1.add QK bias
//2.padding seq_len in k_buf_ to a multiple of 32 named seq_len_padded
//3.transform each Q K row-major matrixes into a series of sub-matrix (with CUBLASLT_ORDER_COL32/CUBLASLT_ORDER_COL4_4R2_8C layout)
...
template <typename T>
__global__
void add_QK_bias_transform_varlen_row(int8_t *q_buf_, int8_t *k_buf_, const int8_t* Q, const T* bias_Q, 
                           const int8_t* K, const T* bias_K, const int m, const int batch_size, 
                           const int seq_len, const int head_num, const int size_per_head, 
                           const int seq_len_padded, const int stride_q, const int stride_k,
                           const float *q_input_deQFactor_ptr, const float *k_input_deQFactor_ptr, 
...
template <typename T>
void invokeAddQKBiasTransformRow(int8_t *q_buf, int8_t *k_buf, const int8_t* Q, const T* bias_Q, 
                                 const int8_t* K, const T* bias_K, const int batch_size, 
                                 const int seq_len, const int head_num, const int size_per_head, 
                                 const float *q_input_deQFactor_ptr, const float *k_input_deQFactor_ptr, 
                                 const float *q_output_scale_ptr, const float *k_output_scale_ptr,
                                 bool use_ORDER_COL32_2R_4R4, cudaStream_t stream) 
{
  ...
  if (seq_len % 32 == 0)
  {
    add_QK_bias_transform_varlen_row<<<dim3(batch_size*seq_len*2), dim3((head_num*size_per_head)/4), 0, stream>>>(
      q_buf, k_buf, Q, bias_Q, K, bias_K, 
      batch_size * seq_len, batch_size, seq_len, head_num, size_per_head, 
      seq_len, seq_len*size_per_head, seq_len*size_per_head,
      q_input_deQFactor_ptr, k_input_deQFactor_ptr, q_output_scale_ptr, k_output_scale_ptr,
      use_ORDER_COL32_2R_4R4, head_num*size_per_head);
  }
  else
  {
    int seq_len_padded = (seq_len + 31)/32*32;
...
template
void invokeAddQKBiasTransformRow(int8_t *q_buf, int8_t *k_buf, const int8_t* Q, const float* bias_Q, 
                                 const int8_t* K, const float* bias_K, const int batch_size, 
                                 const int seq_len, const int head_num, const int size_per_head, 
                                 const float *q_input_deQFactor_ptr, const float *k_input_deQFactor_ptr, 
                                 const float *q_output_scale_ptr, const float *k_output_scale_ptr,
...
template
void invokeAddQKBiasTransformRow(int8_t *q_buf, int8_t *k_buf, const int8_t* Q, const half* bias_Q, 
                                 const int8_t* K, const half* bias_K, const int batch_size, 
                                 const int seq_len, const int head_num, const int size_per_head, 
                                 const float *q_input_deQFactor_ptr, const float *k_input_deQFactor_ptr, 
                                 const float *q_output_scale_ptr, const float *k_output_scale_ptr,
...
template <typename T>
void invokeAddQKBiasTransformRebuildPadding(int8_t *q_buf, int8_t *k_buf, 
...
template <typename T>
void invokeAddQKBiasTransformRebuildPadding(int8_t *q_buf, int8_t *k_buf, const int8_t* Q, const T* bias_Q, 
                                                          const int8_t* K, const T* bias_K, const int* sequence_id_offset, 
...
//add_QK_bias_transform & rebuild padding for batch int8 cublasLtMatmul & per tensor quantization for weight
//1.add QK bias
//2.transform each Q K row-major matrixes into a series of sub-matrix (with CUBLASLT_ORDER_COL32/CUBLASLT_ORDER_COL4_4R2_8C layout)
//  Q, K are CUBLASLT_ORDER_COL32 matrixes of m = valid_word_num, n = head_num * size_per_head
//  q_buf_ is of batchCount = batch_size * head_num, m = seq_len, n = size_per_head, CUBLASLT_ORDER_COL32
//  seq_len_padded = (seq_len + 31)/32*32;
//  k_buf_ is of batchCount = batch_size * head_num, m = seq_len_padded, n = size_per_head, CUBLASLT_ORDER_COL4_4R2_8C or CUBLASLT_ORDER_COL32_2R_4R4
...
template <typename T>
__global__
void add_QK_bias_transform_rebuild_padding_varlen_row(int8_t *q_buf_, int8_t *k_buf_, const int8_t* Q, const T* bias_Q,
                                                      const int8_t* K, const T* bias_K, const int* sequence_id_offset,
                                                      const int valid_word_num, const int m, const int batch_size, 
                                                      const int seq_len, const int seq_len_padded, const int head_num,  
                                                      const int size_per_head, int stride_q, int stride_k,
...
template <typename T>
void invokeAddQKBiasTransformRebuildPaddingRow(int8_t *q_buf, int8_t *k_buf, const int8_t* Q, const T* bias_Q, 
                                               const int8_t* K, const T* bias_K, const int* sequence_id_offset, 
                                               const int valid_word_num, 
                                               const int batch_size, const int seq_len, 
                                               const int head_num, const int size_per_head,  
...
{
  int seq_len_padded = (seq_len + 31)/32*32;
  add_QK_bias_transform_rebuild_padding_varlen_row<<<dim3(valid_word_num*2), dim3((head_num*size_per_head)/4), 0, stream>>>(
    q_buf, k_buf, Q, bias_Q, K, bias_K, 
    sequence_id_offset, valid_word_num, 
    batch_size * seq_len, batch_size, 
    seq_len, seq_len_padded, head_num, size_per_head, 
...
template
void invokeAddQKBiasTransformRebuildPaddingRow(int8_t *q_buf, int8_t *k_buf, 
                                               const int8_t* Q, const float* bias_Q, 
                                               const int8_t* K, const float* bias_K, 
                                               const int* sequence_id_offset, const int valid_word_num, 
                                               const int batch_size, const int seq_len, 
...
template <typename T>
void invokeAddVBiasTransform(int8_t *v_buf, const int32_t *V, const T *V_bias, 
                             const int batch_size, const int seq_len, 
                             const int head_num, const int size_per_head, 
                             const float* weight_amax, 
...
template 
void invokeAddVBiasTransform(int8_t *v_buf, const int32_t *V, const float *V_bias, 
                             const int batch_size, const int seq_len, 
                             const int head_num, const int size_per_head, 
                             const float* weight_amax, 
                             const float *input_deQFactor_div127_ptr, const float *out_scale_ptr, 
...
template 
void invokeAddVBiasTransform(int8_t *v_buf, const int32_t *V, const half *V_bias, 
                             const int batch_size, const int seq_len, 
                             const int head_num, const int size_per_head, 
                             const float* weight_amax, 
                             const float *input_deQFactor_div127_ptr, const float *out_scale_ptr, 
...
template <typename T>
void invokeAddVBiasTransform(int8_t *v_buf, const int8_t *V, const T *V_bias, const int batch_size, 
                             const int seq_len, const int head_num, const int size_per_head,
...
template
void invokeAddVBiasTransform(int8_t *v_buf, const int8_t *V, const float *V_bias, const int batch_size, 
                             const int seq_len, const int head_num, const int size_per_head,
                             const float *input_deQFactor_ptr, const float *out_scale_ptr, 
                             bool use_ORDER_COL32_2R_4R4, cudaStream_t stream); 

template
void invokeAddVBiasTransform(int8_t *v_buf, const int8_t *V, const half *V_bias, const int batch_size, 
                             const int seq_len, const int head_num, const int size_per_head,
                             const float *input_deQFactor_ptr, const float *out_scale_ptr, 
                             bool use_ORDER_COL32_2R_4R4, cudaStream_t stream);                                          
...
template <typename T>
__global__
void add_V_bias_transform_varlen_row(int8_t *v_buf_, const int8_t *V, const T *V_bias, const int batch_size, const int seq_len, 
                                     const int head_num, const int size_per_head, const int seq_len_padded, int stride,
                                     const float *input_deQFactor_ptr, const float *out_scale_ptr, bool use_ORDER_COL32_2R_4R4, const int head_num_x_size_per_head)
{
  const float input_deQFactor = __ldg(input_deQFactor_ptr);
  const float out_scale = __ldg(out_scale_ptr);
  __shared__ int8_t shm[32][33];
...
template <typename T>
void invokeAddVBiasTransformRow(int8_t *v_buf, const int8_t *V, const T *V_bias, const int batch_size, 
                                const int seq_len, const int head_num, const int size_per_head,
                                const float *input_deQFactor_ptr, const float *out_scale_ptr, 
                                bool use_ORDER_COL32_2R_4R4, cudaStream_t stream)
{
  ...
  if (seq_len % 32 == 0)
  {
    add_V_bias_transform_varlen_row<<<dim3(size_per_head/32, seq_len/32, batch_size*head_num), dim3(8, 32), 0, stream>>>(
      v_buf, V, V_bias, 
      batch_size, seq_len, head_num, size_per_head, 
      seq_len, seq_len*size_per_head,
      input_deQFactor_ptr, out_scale_ptr, use_ORDER_COL32_2R_4R4, head_num*size_per_head);
  }
  else
  {
    const int seq_len_padded = (seq_len + 31)/32*32;
    add_V_bias_transform_varlen_row<<<dim3(size_per_head/32, seq_len_padded/32, batch_size*head_num), dim3(8, 32), 0, stream>>>(
      v_buf, V, V_bias, 
      batch_size, seq_len, head_num, size_per_head, 
      seq_len_padded, seq_len_padded*size_per_head,
      input_deQFactor_ptr, out_scale_ptr, use_ORDER_COL32_2R_4R4, head_num*size_per_head);
...
template
void invokeAddVBiasTransformRow(int8_t *v_buf, const int8_t *V, const float *V_bias, const int batch_size, 
                                const int seq_len, const int head_num, const int size_per_head,
                                const float *input_deQFactor_ptr, const float *out_scale_ptr, 
                                bool use_ORDER_COL32_2R_4R4, cudaStream_t stream); 

template
void invokeAddVBiasTransformRow(int8_t *v_buf, const int8_t *V, const half *V_bias, const int batch_size, 
                                const int seq_len, const int head_num, const int size_per_head,
                                const float *input_deQFactor_ptr, const float *out_scale_ptr, 
                                bool use_ORDER_COL32_2R_4R4, cudaStream_t stream);
...
template <typename T>
void invokeAddVBiasTransformRebuildPadding(int8_t *v_buf, const int32_t *V, const T *V_bias, 
...
template 
void invokeAddVBiasTransformRebuildPadding(int8_t *v_buf, const int32_t *V, const float *V_bias, 
                                           const int* sequence_id_map, const int valid_word_num, 
                                           const int batch_size, const int seq_len, 
                                           const int head_num, const int size_per_head, 
                                           const float* weight_amax, 
...
template 
void invokeAddVBiasTransformRebuildPadding(int8_t *v_buf, const int32_t *V, const half *V_bias, 
                                           const int* sequence_id_map, const int valid_word_num, 
                                           const int batch_size, const int seq_len, 
                                           const int head_num, const int size_per_head, 
                                           const float* weight_amax, 
...
template <typename T>
void invokeAddVBiasTransformRebuildPadding(int8_t *v_buf, const int8_t *V, const T *V_bias, 
                                           const int* sequence_id_map, const int valid_word_num, 
                                           const int batch_size, const int seq_len, 
                                           const int head_num, const int size_per_head,
...
template
void invokeAddVBiasTransformRebuildPadding(int8_t *v_buf, const int8_t *V, const float *V_bias, 
                                           const int* sequence_id_map, const int valid_word_num, 
                                           const int batch_size, const int seq_len, 
                                           const int head_num, const int size_per_head,
                                           const float *deQFactor_ptr, const float *out_scale_ptr, 
...
template
void invokeAddVBiasTransformRebuildPadding(int8_t *v_buf, const int8_t *V, const half *V_bias, 
                                           const int* sequence_id_map, const int valid_word_num, 
                                           const int batch_size, const int seq_len, 
                                           const int head_num, const int size_per_head,
                                           const float *deQFactor_ptr, const float *out_scale_ptr, 
...
template <typename T>
__global__
void add_V_bias_transform_rebuild_padding_varlen_row(int8_t *v_buf_, const int8_t *V, const T *V_bias, const int* sequence_id_map, const int valid_word_num,
                                                 const int batch_size, const int seq_len, const int seq_len_padded, 
                                                 const int head_num, const int size_per_head, int stride,
                                                 const float *deQFactor_ptr, const float *out_scale_ptr, bool use_ORDER_COL32_2R_4R4, const int head_num_x_size_per_head)
{
  __shared__ int8_t shm[32][33];
  const char4* data_ptr = (const char4*)V;
  char4* buf_ptr4 = (char4*) v_buf_;
...
template <typename T>
void invokeAddVBiasTransformRebuildPaddingRow(int8_t *v_buf, const int8_t *V, const T *V_bias, 
                                              const int* sequence_id_map, const int valid_word_num, 
                                              const int batch_size, const int seq_len, 
                                              const int head_num, const int size_per_head,
                                              const float *deQFactor_ptr, const float *out_scale_ptr, 
...
{
  int seq_len_padded = (seq_len + 31)/32*32;
  add_V_bias_transform_rebuild_padding_varlen_row<<<dim3(size_per_head/32, seq_len_padded/32, batch_size*head_num), dim3(8, 32), 0, stream>>>(
    v_buf, V, V_bias, sequence_id_map, valid_word_num, 
    batch_size, seq_len, seq_len_padded, head_num, size_per_head, seq_len_padded*size_per_head, 
    deQFactor_ptr, out_scale_ptr, use_ORDER_COL32_2R_4R4, head_num*size_per_head);
}           

template
void invokeAddVBiasTransformRebuildPaddingRow(int8_t *v_buf, const int8_t *V, const float *V_bias, 
                                              const int* sequence_id_map, const int valid_word_num, 
                                              const int batch_size, const int seq_len, 
                                              const int head_num, const int size_per_head,
                                              const float *deQFactor_ptr, const float *out_scale_ptr, 
...
template
void invokeAddVBiasTransformRebuildPaddingRow(int8_t *v_buf, const int8_t *V, const half *V_bias, 
                                              const int* sequence_id_map, const int valid_word_num, 
                                              const int batch_size, const int seq_len, 
                                              const int head_num, const int size_per_head,
                                              const float *deQFactor_ptr, const float *out_scale_ptr, 
