...
template<> struct Float_as_int_<float>  { using Type = uint32_t; };
template<> struct Float_as_int_<__half> { using Type = uint16_t; };
...
__global__ __launch_bounds__(Kernel_params::BLOCK_THREADS, 1)
void segmented_top_p_single_pass(TopKPerSegmentParams params)
{
#if DO_DEBUG_PRINT
...
// Load all keys into registers and smem
    ...
    for (Int_Key_Data_Type bit = START_BIT - SKIP_BITS; true; --bit) {
        ...
        if ((p_sum_total <= params.top_p + P_EPSILON && p_sum_total > 0) || 
            (p_sum_total > params.top_p && sc <= MAX_TOP_K ) ||
...
__syncthreads();
            ...
            for (int item = 0; item < ITEMS_PER_THREAD_IN_REGS; ++item) { 
                ...
                if (enabled[item / 32] & (ONE << (item % 32)) && offset < MAX_TOP_K) { 
                    smem_selected_elements[offset] = make_int2(thread_items[item], item + threadIdx.x * ITEMS_PER_THREAD); 
                    ++offset;
            ...
            for (int item = 0; item < ITEMS_PER_THREAD_IN_SMEM; ++item) {
                if (enabled[(item + ITEMS_PER_THREAD_IN_REGS) / 32] & (ONE << ((item + ITEMS_PER_THREAD_IN_REGS) % 32)) && offset < MAX_TOP_K) {
                    int idx = threadIdx.x + item * BLOCK_THREADS;
                    //int idx = item + ITEMS_PER_THREAD_IN_SMEM * threadIdx.x;
                    //if (idx <  params.num_items_per_segment_in_smem) 
                    {
                        smem_selected_elements[offset] = make_int2(smem_thread_items[idx], item + threadIdx.x * ITEMS_PER_THREAD + ITEMS_PER_THREAD_IN_REGS); 
                        ++offset;
...
int getSmemSizeAndCheck(const TopKPerSegmentContext& context, const TopKPerSegmentParams& params) {
    ...
    smem_size +=  items_per_thread_in_smem *
                  BLOCK_THREADS * sizeof(typename Float_as_int_<Key_Data_Type>::Type);

    int keys_per_ldg = 2 * sizeof(Key_Data_Type) / 2;
    if ( smem_size + BLOCK_THREADS * sizeof(float) > context.sm_shared_size || // dynamic + static memory
         items_per_thread_in_regs + items_per_thread_in_smem != items_per_thread || 
         params.top_p + P_EPSILON > 1.0F ||
         items_per_thread_in_regs % keys_per_ldg != 0 ||
         items_per_thread_in_smem % keys_per_ldg != 0 ||
        ...
        if( smem_size > 0 ) \
            check_cuda_error(cudaFuncSetAttribute( \
                segmented_top_p_single_pass< \
                    Kernel_params,          \
                    ITEMS_INCREMENT * (INDEX + 1)>, \
                cudaFuncAttributeMaxDynamicSharedMemorySize, \
                smem_size));                                 \
            segmented_top_p_single_pass< \
                Kernel_params, \
                ITEMS_INCREMENT * (INDEX + 1)><<<grid_dim,Kernel_params::BLOCK_THREADS, smem_size, stream>>>(params); }
...
void topPPerSegment_dispatch(
    ...
    if (temp_storage == nullptr ) {
        if (params.num_segments > 1) {
            cub::DeviceSegmentedRadixSort::SortPairsDescending(temp_storage, temp_storage_bytes,
                ...
                params.num_items, params.num_segments, 
            ...
            cub::DeviceRadixSort::SortPairsDescending(temp_storage, temp_storage_bytes,
                ...
                reinterpret_cast<Value_Data_Type *>(params.gmem_src_vals), 
                reinterpret_cast<Value_Data_Type *>(params.gmem_dst_vals),
                params.num_items, 0, sizeof(Key_Data_Type) * 8, stream);
        }
    ...
    size_t cub_temp_storage_bytes = temp_storage_bytes - div_up(sizeof(int), 256) * 256 - div_up(sizeof(int) * params.num_segments, 256) * 256;
    ...
    params.gmem_active_count_per_segment = reinterpret_cast<int *>((char *) params.gmem_active_count_total + div_up(sizeof(int), 256) * 256);
    ...
    if (max_num_items >= MAX_TOP_K || max_num_items == 0) {
        ...
        blockSort<Key_Data_Type>(   
            (const Key_Data_Type*) (params.gmem_dst_keys), (Key_Data_Type*) (params.gmem_dst_keys),
            (const Value_Data_Type*) (params.gmem_dst_vals), (Value_Data_Type*) (params.gmem_dst_vals), 
            params.gmem_active_count_per_segment,
            max_num_items, num_items_per_segment, params.num_segments, stream
        );
    }
    ...
    if (begin_offset_buf[blockIdx.x] == offset_buf[blockIdx.x] ) {
        if (threadIdx.x == 0) {
            ...
            if (sequence_length != nullptr && finished_buf != nullptr) {
                ...
                finished_buf[blockIdx.x] = ids[blockIdx.x] == end_id ? 1 : 0;
            }
        }
        return;
    }
                        ...
                        cudaDeviceProp *cuda_device_prop)
{
    ...
    segmented_topp_impl::DType_t dataTypeKind = (std::is_same<T, float>::value)? segmented_topp_impl::kFLOAT : segmented_topp_impl::kHALF;
    ...
    if (do_radix_sort) {
        if (workspace == nullptr) {
            ...
            workspace_size =
                sorted_log_prob_buf_size + sorted_id_vals_buf_size + cub_temp_storage_size;
            return;
        }
        ...
        if (workspace == nullptr) {
            segmented_topp_impl::topPPerSegment(context, params, dataTypeKind, cub_temp_storage, cub_temp_storage_size, stream);
            workspace_size =
                sorted_log_prob_buf_size + sorted_id_vals_buf_size + cub_temp_storage_size;
            return;
        } else {
...
template<typename T>
__global__ void
addBiasSoftMax(T* logits, const T* bias, const int end_id, const bool* finished, const int n_padded, const int n)
{
    int bid = blockIdx.x;
                          ...
                          cudaStream_t stream)
{
    ...
    addBiasSoftMax<<<grid, block, 0, stream>>>(logits, bias, end_id, finished, n_padded, n);
}
