...
__global__
void quantize_weight_kernel(int8_t *dst, const float* src, const float* amax, const int n, const int k, const int format) {
    ...
    if (tid < n * k) {
        int8_t v = float_to_int8_rn(src[tid] * 127.0 / amax[row_idx]);
        dst[new_idx] = v;
    }
}

__global__
void quantize_weight_kernel(int8_t *dst, const half* src, const float* amax, const int n, const int k, const int format) {
    int tid = (blockIdx.x*blockDim.x + threadIdx.x);
    int col_idx = tid / n;
    int row_idx = tid - col_idx * n;
    int new_idx;
    ...
    if (tid < n * k) {
        int8_t v = float_to_int8_rn(__half2float(src[tid]) * 127.0 / amax[row_idx]);
        dst[new_idx] = v;
    }
}
...
void invokeQuantizeWeight(int8_t* dst, const T* src, const float* amax, const int n, const int k, const int format, cudaStream_t stream) {
    ...
    if (sizeof(T) == sizeof(float))
        quantize_weight_kernel<<<grid, block, 0, stream>>>(dst, (const float*)src, amax, n, k, format);
    else if (sizeof(T) == sizeof(half))
        quantize_weight_kernel<<<grid, block, 0, stream>>>(dst, (const half*)src, amax, n, k, format);
 }

template
void invokeQuantizeWeight<float>(int8_t* dst, const float* src, const float* amax, const int n, const int k, const int format, cudaStream_t stream);

template
void invokeQuantizeWeight<half>(int8_t* dst, const half* src, const float* amax, const int n, const int k, const int format, cudaStream_t stream);
